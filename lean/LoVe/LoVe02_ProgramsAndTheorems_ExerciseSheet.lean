...
#eval eval someEnv (AExp.add (AExp.var "x") (AExp.var "y"))
...
theorem identity_map {α: Type} (l: List α):
  map (fun x => x) l = l := sorry

theorem map_asso {α: Type} {β: Type} {γ: Type} (f: α → β) (g: β → γ) (l: List α):
